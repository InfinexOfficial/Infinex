                                  ...
                                  vector<char>& vfBest, CAmount& nBest, int iterations = 1000, bool fUseInstantSend = false)
{
    ...
    for (int nRep = 0; nRep < iterations && nBest != nTargetValue; nRep++)
    {
        ...
        for (int nPass = 0; nPass < 2 && !fReachedTarget; nPass++)
        {
            ...
            {
                if (fUseInstantSend && nTotal + vValue[i].first > (sporkManager.GetNumericSporkValue(SPORK_INSTANTSEND_MAX_VALUE)*COIN)) {
                    continue;
                }
                                 ...
                                 set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, bool fUseInstantSend) const
{
    ...
    coinLowestLarger.first = fUseInstantSend
                                        ? (sporkManager.GetNumericSporkValue(SPORK_INSTANTSEND_MAX_VALUE)*COIN)
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseInstantSend))
                {
                    if (nCoinType == ONLY_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 INFINEX.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough PrivateSend non-denominated funds for this transaction that are not equal 1000 INFINEX.");
                ...
                if (fUseInstantSend && nValueIn > (sporkManager.GetNumericSporkValue(SPORK_INSTANTSEND_MAX_VALUE)*COIN)) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 INFINEX."), sporkManager.GetNumericSporkValue(SPORK_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-infinex-address
                        CScript scriptChange;
