...
void CInstantSend::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(!sporkManager.IsSporkActive(SPORK_INSTANTSEND_ENABLED)) return;
...
bool CInstantSend::IsInstantSendReadyToLock(const uint256& txHash)
{
    if(!fEnableInstantSend || fLargeWorkForkFound || fLargeWorkInvalidChainFound ||
        !sporkManager.IsSporkActive(SPORK_INSTANTSEND_ENABLED)) return false;
...
{
    if(!fEnableInstantSend || fLargeWorkForkFound || fLargeWorkInvalidChainFound ||
        !sporkManager.IsSporkActive(SPORK_INSTANTSEND_BLOCK_FILTERING)) return false;
...
int CInstantSend::GetTransactionLockSignatures(const uint256& txHash)
{
    ...
    if(!sporkManager.IsSporkActive(SPORK_INSTANTSEND_ENABLED)) return -3;
...
bool CTxLockRequest::IsValid() const
{
    ...
    if(nValueIn > (sporkManager.GetNumericSporkValue(SPORK_INSTANTSEND_MAX_VALUE)*COIN)) {
