...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        // NOTE: unlike in bitcoin, we need to pass block height here
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
		...
		if(!IsExceedMinimumChainWork())
			return error("ProcessBlockFound -- chain tip work lesser than minimum chain work");

		if(!IsBestBlockHashFound())
			return error("ProcessBlockFound -- best block hash not found in chain");
    }
...
{
    LogPrintf("InfinexMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("infinex-miner");
        ...
        while (true) {
            if (chainparams.MiningRequiresPeers()) {
                ...
                do {
                    ...
                    if (!fvNodesEmpty && !IsInitialBlockDownload() && masternodeSync.IsSynced() && IsExceedMinimumChainWork() && IsBestBlockHashFound())
                        break;
                    MilliSleep(1000);
                } while (true);
            }
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("InfinexMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("InfinexMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("InfinexMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("InfinexMiner -- runtime error: %s\n", e.what());
        return;
    }
}

void GenerateInfinex(bool fGenerate, int nThreads, const CChainParams& chainparams, CConnman& connman)
{
