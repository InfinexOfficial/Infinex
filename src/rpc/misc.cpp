...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total infinex balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized infinex balance of the wallet\n"
...
UniValue debug(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "infinex|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use comma to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "infinex")
            + HelpExampleRpc("debug", "infinex,net")
        );
...
UniValue spork(const UniValue& params, bool fHelp)
{
	if (params.size() == 1 && params[0].get_str() == "show") {
		...
		for (int nSporkID = SPORK_START; nSporkID <= SPORK_END; nSporkID++) {
			...
			int SPORK_TYPE = sporkManager.GetSporkType(nSporkID);
			if (SPORK_TYPE == SPORK_UNKNOWN_TYPE) {
				ret.push_back(Pair(sporkManager.GetSporkNameByID(nSporkID), "Unknown"));
			}
			else if (SPORK_TYPE == SPORK_BOOL_TYPE) {
				string status = "DISABLED";
				if (sporkManager.GetNumericSporkValue(nSporkID) == SPORK_ENABLED) {
					status = "ENABLED";
				}
				ret.push_back(Pair(sporkManager.GetSporkNameByID(nSporkID), status));
			}
			else if (SPORK_TYPE == SPORK_NUMERIC_TYPE) {
				ret.push_back(Pair(sporkManager.GetSporkNameByID(nSporkID), sporkManager.GetNumericSporkValue(nSporkID)));
			}
			else if (SPORK_TYPE == SPORK_TEXT_TYPE) {
				ret.push_back(Pair(sporkManager.GetSporkNameByID(nSporkID), sporkManager.GetTextSporkValue(nSporkID)));
			}
		}
		return ret;
	}
		...
		int SPORK_TYPE = sporkManager.GetSporkType(nSporkID);
		if (SPORK_TYPE == SPORK_UNKNOWN_TYPE) {
			return "Invalid spork type";
		}
		else if (SPORK_TYPE == SPORK_NUMERIC_TYPE || SPORK_TYPE == SPORK_BOOL_TYPE || SPORK_TYPE == SPORK_TIME_TYPE) {
			int64_t nValue = params[1].get_int64();
			if (sporkManager.UpdateNumericSpork(nSporkID, nValue, *g_connman)) {
				sporkManager.ExecuteNumericSpork(nSporkID, nValue);
				return "success";
			}
			else {
				return "failure";
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"infinexaddress\"\n"
            "\nReturn information about the given infinex address.\n"
            "\nArguments:\n"
            "1. \"infinexaddress\"     (string, required) The infinex address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"infinexaddress\", (string) The infinex address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Infinex address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are infinex addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) infinex address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"infinexaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"infinexaddress\"  (string, required) The infinex address to use for the signature.\n"
