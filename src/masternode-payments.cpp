...
bool IsBlockValueValid(const CBlock& block, int nBlockHeight, CAmount blockReward, std::string &strErrorRet)
{
	...
	CAmount currentBlockReward=0;
	currentBlockReward=block.vtx[0].GetValueOut();

	bool isBlockRewardValueMet = (currentBlockReward <= blockReward);
	if (fDebug) LogPrintf("Currentblockreward %lld <= blockReward %lld\n", currentBlockReward, blockReward);
...
{	
	...
	CAmount masternodePayment = GetMasternodeReward(nBlockHeight);
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
	...
	CAmount masternodeReward = GetMasternodeReward(nBlockHeight);
	...
	BOOST_FOREACH(CMasternodePayee& payee, vecPayees) {
		if (payee.GetVoteCount() >= MNPAYMENTS_SIGNATURES_REQUIRED) {
			BOOST_FOREACH(CTxOut txout, txNew.vout) {
				if (payee.GetPayee() == txout.scriptPubKey && masternodeReward == txout.nValue) {
	...
	LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f INFINEX\n", strPayeesPossible, (float)masternodeReward / COIN);
	return false;
}
...
void CMasternodePaymentVote::Relay(CConnman& connman)
{
	...
	connman.RelayInv(inv, mnpayments.GetMinMasternodePaymentsProto());
}
