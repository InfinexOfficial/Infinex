...
#if defined(NDEBUG)
#error "Infinex Core cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Infinex Signed Message:\n";
...
CAmount GetBlockSubsidy(int nHeight)
{
    ...
    if (nHeight > (halvingInterval * 10))
        return 0 * COIN;
    int halvingRound = nHeight / halvingInterval;
    if (halvingRound >= 5)
        return 6 * COIN;
    return (11 - halvingRound) * COIN;
}

CAmount GetMasternodeReward(int nHeight)
{
    int halvingInterval = Params().GetConsensus().nSubsidyHalvingInterval;
    if (nHeight > (halvingInterval * 10))
        return 0 * COIN;
...
bool IsInitialBlockDownload()
{
    ...
    if (!IsExceedMinimumChainWork(true)) {
        return true;
    }
...
bool IsExceedMinimumChainWork(bool reCheckExceedMinimumChainWork)
{
    static bool EXCWState = false;

    if (EXCWState && !reCheckExceedMinimumChainWork) {
        return true;
    }

    if (chainActive.Tip()->nChainWork < UintToArith256(uint256S(sporkManager.GetTextSporkValue(SPORK_MINIMUM_CHAIN_WORK)))) {
        EXCWState = false;
        return false;
    }
...
bool IsBestBlockHashFound(bool reCheckBestBlockHashFound)
{
    static bool BBHFState = false;

    if (BBHFState && !reCheckBestBlockHashFound) {
        return true;
    }

    uint256 hash(uint256S(sporkManager.GetTextSporkValue(SPORK_BEST_BLOCK_HASH)));
    if (mapBlockIndex.count(hash) < 1) {
        BBHFState = false;
        return false;
    }
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            if (pvChecks) {
                ...
                if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                    ...
                    // non-upgraded nodes.
                    CScriptCheck check2(*coins, tx, i, flags & ~STANDARD_NOT_MANDATORY_VERIFY_FLAGS, cacheStore);
                    if (check2())
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    BlockMap::const_iterator it = mapBlockIndex.find(uint256S(sporkManager.GetTextSporkValue(SPORK_BEST_BLOCK_HASH)));
    if (it != mapBlockIndex.end()) {
        if (it->second->GetAncestor(pindex->nHeight) == pindex &&
            pindexBestHeader->GetAncestor(pindex->nHeight) == pindex &&
            pindexBestHeader->nChainWork >= UintToArith256(uint256S(sporkManager.GetTextSporkValue(SPORK_MINIMUM_CHAIN_WORK)))) {
    ...
    // INFINEX : MODIFIED TO CHECK MASTERNODE PAYMENTS
    CAmount blockReward = nFees + GetBlockSubsidy(pindex->nHeight);
    std::string strError = "";
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        //bad coinbase amount
        return state.DoS(0, error("ConnectBlock(INFINEX): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    CAmount masternodeReward = GetMasternodeReward(pindex->nHeight);
    if(masternodeReward>0) {
        if (pindex->nHeight > Params().GetConsensus().nMasternodePaymentsStartBlock && !IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
            ...
            return state.DoS(0, error("ConnectBlock(INFINEX): couldn't find masternode payments"), REJECT_INVALID, "bad-cb-payee");
        }
    }
    // END INFINEX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // INFINEX : CHECK TRANSACTIONS FOR INSTANTSEND

    if (sporkManager.IsSporkActive(SPORK_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH (const CTxIn& txin, tx.vin) {
                ...
                if (instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(INFINEX): transaction %s conflicts with transaction lock %s", tx.GetHash().ToString(), hashLocked.ToString()),
                        REJECT_INVALID, "conflict-tx-lock");
                }
            }
        }
    } else {
        LogPrintf("CheckBlock(INFINEX): spork is off, skipping transaction locking checks\n");
    }
