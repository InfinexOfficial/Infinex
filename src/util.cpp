...
const char * const BITCOIN_CONF_FILENAME = "infinex.conf";
const char * const BITCOIN_PID_FILENAME = "infinexd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "infinex" is a composite category enabling all Infinex-related debug output
            if(ptrCategory->count(string("infinex"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\InfinexCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\InfinexCore
    // Mac: ~/Library/Application Support/InfinexCore
    // Unix: ~/.infinexcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "InfinexCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/InfinexCore";
#else
    // Unix
    return pathRet / ".infinexcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty infinex.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override infinex.conf
