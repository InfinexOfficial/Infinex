...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered infinex address / amount:
...

    {
        ...
        if(recipients[0].fUseInstantSend && total > (sporkManager.GetNumericSporkValue(SPORK_INSTANTSEND_MAX_VALUE)*COIN)) {
            Q_EMIT message(tr("Send Coins"), tr("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 INFINEX.").arg(sporkManager.GetNumericSporkValue(SPORK_INSTANTSEND_MAX_VALUE)),
        ...
        if(recipients[0].fUseInstantSend) {
            if(newTx->GetValueOut() > (sporkManager.GetNumericSporkValue(SPORK_INSTANTSEND_MAX_VALUE)*COIN)) {
                Q_EMIT message(tr("Send Coins"), tr("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 INFINEX.").arg(sporkManager.GetNumericSporkValue(SPORK_INSTANTSEND_MAX_VALUE)),
                             CClientUIInterface::MSG_ERROR);
                return TransactionCreationFailed;
            }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal infinex:URI (infinex:XyZ...?message=example)
            {
