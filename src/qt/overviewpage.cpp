...
public:
    TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(BitcoinUnits::IFX),
    ...
    txdelegate(new TxViewDelegate(platformStyle, this))
{
    ...
    if(fMasterNode || nWalletBackups <= 0){
		...
		//connect(timer, SIGNAL(timeout()), this, SLOT(masternodeStatus()));
        timer->start(1000);
    }
}
...
OverviewPage::~OverviewPage()
{
	...
	//if (!fLiteMode && !fMasterNode) disconnect(timer, SIGNAL(timeout()), this, SLOT(masternodeStatus()));
    delete ui;
}
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    {
        // update the display unit, to not use the default ("INFINEX")
        updateDisplayUnit();
...
//void OverviewPage::masternodeStatus() 
//{
//	if (!masternodeSync.IsSynced() || ShutdownRequested()) return;
//
//	static int64_t countdownTimer = GetTime();
//	int64_t timeToRun = countdownTimer - GetTime() + 60;
//	if (timeToRun > 0)
//		return;
//
//	countdownTimer = GetTime();
//
//	int globalMasternodesTotal = mnodeman.CountMasternodes();
//	int globalMasternodesEnabled = mnodeman.CountEnabled();
//	if (globalMasternodesTotal > -1 && globalMasternodesEnabled > -1) {
//		ui->lblGlobalMasternodesCount->setText(QString::number(globalMasternodesEnabled) + " / " + QString::number(globalMasternodesTotal));
//	}
//
//	int yourMasternodesTotal = 0;
//	int yourMasternodesEnabled = 0;
...
//		}
//
//		yourMasternodesTotal += 1;
//		masternode_info_t infoMn;
//		bool fFound = mnodeman.GetMasternodeInfo(nOutputIndex, infoMn);
//		if (fFound) {
//			if (infoMn.nActiveState == 1) {
//				yourMasternodesEnabled += 1;
//			}
//		}
//	}
//	ui->lblYourMasternodesCount->setText(QString::number(yourMasternodesEnabled) + " / " + QString::number(yourMasternodesTotal));
//	if (yourMasternodesEnabled < 1 || globalMasternodesEnabled < 1)
//		ui->lblExpectedReturn->setText("0.00 IFX");
//	else
//	{
//		float expectedReturn = (float)4800 * (float)yourMasternodesTotal / (float)globalMasternodesTotal;
//		ui->lblExpectedReturn->setText(QString::number(expectedReturn) + " Infinex");
//	}
//}
