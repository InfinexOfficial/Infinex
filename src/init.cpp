...
#include "InfiniDEX/activenoderole.h"
#ifdef ENABLE_WALLET
#include "wallet/db.h"
...
#endif
	GenerateInfinex(false, 0, Params(), *g_connman);
    MapPort(false);
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, http, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "infinex (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Infinex specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N INFINEX anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
    ...
    strUsage += HelpMessageOpt("-instantsenddepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-12880, default: %u)"), DEFAULT_INSTANTSEND_DEPTH));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The Infinex Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Infinex Core is running in a usable environment with all
...
void InitParameterInteraction()
{
    ...
    if (nLiqProvTmp > 0) {
        ...
        mapArgs["-privatesendrounds"] = "128809";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -privatesendrounds=128809\n", __func__, nLiqProvTmp);
        mapArgs["-privatesendamount"] = "1288099";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -privatesendamount=1288099\n", __func__, nLiqProvTmp);
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Infinex Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize Infinex Core.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Infinex Core is shutting down."));
...
#endif
    // Make sure only a single Infinex Core process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Infinex Core is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Infinex Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Infinex Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Infinex Core to complete") << "\n";
    ...
    privateSendClient.nPrivateSendRounds = std::min(std::max((int)GetArg("-privatesendrounds", DEFAULT_PRIVATESEND_ROUNDS), 2), privateSendClient.nLiquidityProvider ? 128809 : 16);
    privateSendClient.nPrivateSendAmount = std::min(std::max((int)GetArg("-privatesendamount", DEFAULT_PRIVATESEND_AMOUNT), 2), 1288099);
    ...
    // ********************************************************* Step 11c: update block tip in Infinex modules
    ...
    // ********************************************************* Step 11d: start infinex-ps-<smth> threads
	...
	GenerateInfinex(GetBoolArg("-gen", DEFAULT_GENERATE), GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams, connman);
